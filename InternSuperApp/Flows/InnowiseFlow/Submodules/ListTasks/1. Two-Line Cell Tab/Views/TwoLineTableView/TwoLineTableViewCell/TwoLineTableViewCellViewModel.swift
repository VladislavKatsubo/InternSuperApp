//
//  TwoLineTableViewCellViewModel.swift
//  InternSuperApp
//
//  Created by Vlad Katsubo on 3.08.23.
//

import Foundation

protocol TwoLineTableViewCellViewModelProtocol {
    var onUserImageData: ((Data) -> Void)? { get set }
    var onUserInfo: ((TwoLineTableViewCell.Model) -> Void)? { get set }
...
final class TwoLineTableViewCellViewModel: TwoLineTableViewCellViewModelProtocol {
    ...
    var onUserInfo: ((TwoLineTableViewCell.Model) -> Void)?

    private var dataTask: URLSessionDataTask?

    // MARK: - Configure
    init(model: TwoLineTableViewCell.Model, imageManager: ImageManagerProtocol) {
        self.model = model
...
private extension TwoLineTableViewCellViewModel {
    ...
    func setupUserInfo(with model: TwoLineTableViewCell.Model) {
        onUserInfo?(model)
    }
}
